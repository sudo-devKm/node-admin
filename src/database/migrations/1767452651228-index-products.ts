...
export class IndexProducts1767452651228 implements MigrationInterface {
    name = 'IndexProducts1767452651228'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`products\` (\`id\` varchar(36) NOT NULL DEFAULT '(UUID())', \`created_at\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`updated_at\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, \`title\` varchar(255) NOT NULL, \`description\` varchar(255) NOT NULL, \`image\` json NULL, \`price\` int NOT NULL, INDEX \`products_title_idx\` (\`title\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`DROP INDEX \`products_title_idx\` ON \`products\``);
